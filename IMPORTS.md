...
│   │   └── room_variants.json            Optional: flag-gated room swaps within this region
...
## Flag-gated room variants

A region can author `Regions/<region>/room_variants.json` to swap rooms
based on planet or global flags. The file is optional — when missing the
region behaves as if no variants exist. Shape:
...
```json
{
  ...
  "variants": {
    "<canonical_room_id>": [
      {
        "when": { "scope": "planet", "flag": "town_burned", "equals": true },
        "use":  "town_square_burned"
      }
    ]
  }
}
...
The canonical room id and every `use` target must also exist in this
region's `rooms.json`. `use` targets cannot themselves have variants
