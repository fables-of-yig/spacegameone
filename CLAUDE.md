...
- **Nebula skin** — all four overlays are themed by `NebulaTheme` (`MV/scripts/console/nebula_theme.gd`), a cached Godot `Theme` built from the Claude Design "Nebula" handoff (`nebulous/` in the repo root): token palette consts, 9-slice armored window frames + cyan capsule buttons + recessed input slots, and the Robyn Brutalist Digital font. Assets live in `Assets/UI/nebula/`. The Theme is partial — unset items fall back to the project default theme. Overlays assign it to their root and use `card_box`/`well_box`/`title_label` builders for ad-hoc nodes. `tools/nebula_smoke.gd` proves it builds from real assets.
