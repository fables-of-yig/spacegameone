...
- Regions can author an optional `room_variants.json` to flag-gate room swaps. **Schema and validator only in this slice** — authoring lands clean on disk and is checked by `content_validator.gd`, but the runtime resolver (`MvRoomManager.resolve_room_addr`), live re-resolve on flag change, and the per-room editor subpanel are still pending. See `IMPORTS.md#flag-gated-room-variants` for the on-disk shape.
