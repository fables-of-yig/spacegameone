...
- Generic enemy body-contact damage runs through the player's authored hurtbox area. Entity defs can author `hp`, `attack_damage`, `contact_damage`, `contact_cooldown`, `move_speed`, `projectile_damage`, `projectile_speed`, `melee_range`, `projectile_range`, `melee_attack_trigger_frame`, and `projectile_attack_trigger_frame`, and common AI leaves use those as defaults when their params omit overrides.
...
- Regions can author an optional `room_variants.json` to flag-gate room swaps. On room entry the runtime checks each rule for the canonical room id, evaluates the `when` clause against the live `PlanetaryInterface` flag state, and loads the first matching `use` room's data instead of the canonical's — but the canonical addr is what doors, the map screen, and save snapshots track, so external state stays stable across hot-swaps. Flipping a flag while the player is inside a room triggers a live re-resolve: the swap is deferred while a dialogue, trigger sequence, scripted camera pan, scripted player move, or pause menu is active, then flushes (capturing player coords, reloading the room, clamping coords to the new bounds, brief screen flash). Authored per room from the env editor's VARIANTS button. See `IMPORTS.md#flag-gated-room-variants` for the on-disk shape.
