...
# Full-screen modal that lists every behavior defined in the active
# pack's behaviors.json (user override layer, falling back to shipped)
# and lets the user click one to assign it to the selected entity.
# Fires `picked(behavior_id)` on selection; `cancelled` on Esc or
# click-outside. A "(none)" row at the top clears the field.

signal picked(behavior_id: String)
signal cancelled
...
func open(pack_id: String, current_id: String = "") -> void:
...
func _draw():
    ...
    draw_string(font, box.position + Vector2(24, 36),
        "Pick behavior", HORIZONTAL_ALIGNMENT_LEFT, -1, 16, UIPanels.TEXT_PANEL)
    draw_string(font, box.position + Vector2(24, 58),
        "Reads behaviors.json from the active campaign.",
    ...
    draw_rect(list_rect, Color(0.05, 0.12, 0.08, 0.92))
    draw_rect(list_rect, Color(0.35, 0.6, 0.45, 0.9), false, 1.0)

    _viewport_h = list_h
    var total_rows: int = _behaviors.size() + 1
    ...
    var none_selected := _current_id == ""
    var none_hover := _none_rect.has_point(mouse_pos)
    var none_bg: Color
    if none_selected:
        none_bg = Color(0.3, 0.55, 0.42, 0.95)
    elif none_hover:
        none_bg = Color(0.18, 0.3, 0.22, 0.85)
    else:
        none_bg = Color(0.1, 0.18, 0.12, 0.4)
    if _none_rect.position.y + _none_rect.size.y >= list_y \
            and _none_rect.position.y <= list_y + list_h:
        draw_rect(_none_rect, none_bg)
        draw_string(font, _none_rect.position + Vector2(10, 20),
            "— none —", HORIZONTAL_ALIGNMENT_LEFT, -1, 12,
            Color(0.95, 1.0, 0.8, 1) if none_selected else Color(0.78, 0.92, 0.84, 1))
        if none_hover:
            EditorTooltip.show_text("Clear the behavior field. The entity will have no AI — appropriate for static decor or items.")

    for i in _behaviors.size():
        var b_v: Variant = _behaviors[i]
        if typeof(b_v) != TYPE_DICTIONARY:
            continue
        var b: Dictionary = b_v
        var id := str(b.get("id", ""))
        var name_str := str(b.get("name", ""))
        var desc := str(b.get("description", ""))
        ...
        var sub := name_str
        if desc != "":
            sub = "%s  —  %s" % [name_str, desc]
        if sub != "":
            var max_chars: int = int((rect.size.x - 20.0) / 5.5)
            if sub.length() > max_chars and max_chars > 3:
                sub = sub.substr(0, max_chars - 3) + "..."
            draw_string(font, rect.position + Vector2(10, 28),
                sub, HORIZONTAL_ALIGNMENT_LEFT, -1, 10,
                Color(0.6, 0.82, 0.7, 1))
        if hover:
            var tip := "Behavior \"%s\"" % id
            if name_str != "":
                tip += " — %s" % name_str
            tip += ". Click to assign it to the active entity. The behavior tree ticks every frame for each entity using it."
    ...
    if _behaviors.is_empty():
        ...
        draw_string(font, Vector2(list_x + 12, list_y + 58 + ROW_H),
            "Open the BEHAVIOR editor to define some,",
            HORIZONTAL_ALIGNMENT_LEFT, -1, 11, Color(0.48, 0.7, 0.58, 1))
        draw_string(font, Vector2(list_x + 12, list_y + 74 + ROW_H),
            "then come back here to bind them.",
            HORIZONTAL_ALIGNMENT_LEFT, -1, 11, Color(0.48, 0.7, 0.58, 1))
    ...
    if cancel_hover:
        EditorTooltip.show_text("Close the picker without changing the entity's behavior. Esc or clicking outside the box does the same thing.")

    draw_string(font, Vector2(box.position.x + 24,
        box.position.y + box.size.y - 12),
        "Click a row to assign.  Esc: cancel",
        HORIZONTAL_ALIGNMENT_LEFT, -1, 10, Color(0.55, 0.78, 0.62, 1))
