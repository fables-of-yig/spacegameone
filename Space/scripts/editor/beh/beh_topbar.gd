...
# Top bar for the behavior editor. Campaign label on the left, ADD
# BEHAVIOR button in the middle, SAVE + CLOSE on the right.
...
func _draw():
    ...
    var pad: float = 18.0
    var label := "CAMPAIGN  %s   —   BEHAVIORS" % editor.pack_id
    ...
    UIPanels.draw_button_bg(self, _add_rect, add_hover,
        Color(0.55, 0.95, 0.75, 1))
    var add_label := "+ ADD BEHAVIOR"
    ...
    draw_string(font, Vector2(_add_rect.position.x + (add_btn_w - add_w) * 0.5,
        ...
        Color(1, 1, 1, 1) if add_hover else Color(0.88, 1.0, 0.92, 1))
    if add_hover:
        EditorTooltip.show_text("Create a new behavior tree. You'll be prompted for an ID — entities reference behaviors by ID from their entity definition.")
    ...
    if close_hover:
        EditorTooltip.show_text("Close the behavior editor and return to the main menu. Unsaved changes will prompt you to save first.")
    ...
    if editor.dirty:
        save_label = "SAVE*"
    if save_hover:
        EditorTooltip.show_text("Save all behavior trees to disk. SAVE* means there are unsaved edits — click to persist.")
