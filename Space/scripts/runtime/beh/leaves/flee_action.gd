...
# Sets horizontal velocity to move the actor away from the nearest
# mv_player. Returns FAILURE when no player is found so a parent
# selector can fall through to a default behavior.
#
# params:
...
func tick(actor: Node, _blackboard: Blackboard) -> int:
    ...
    if best == null:
        return FAILURE
    var speed_default: float = float(actor.move_speed) if "move_speed" in actor else 60.0
    var speed: float = float(_params().get("speed", speed_default))
    var dx: float = (actor as Node2D).global_position.x - best.global_position.x
