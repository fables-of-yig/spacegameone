...
const SCREEN_DEFS: Dictionary = {
	...
	"toast": {
		...
		"doc_description": "Optional authored override for the transient toast cards that the `show_toast` trigger action spawns in `MV/scripts/hud.gd`. When authored, the template replaces the default Panel+Label card and may bind `toast.message` and `toast.style` (plus any standard HUD binding). The runtime instantiates one copy of the template per toast and fades it in/out automatically.",
		"mounted": true,
		"host_id": "mv_hud_overlay",
		"required": false,
	},
...
const KNOWN_GAPS: Array = [
	...
	"Landing on a POI opens the Space-side region picker (`Space/scripts/ui/region_picker_panel.gd`, mounted by `UICoordinator.setup_region_picker`) listing the POI's `planet_data.regions[]`; selection calls `PlanetaryInterface.begin_landing(pack_id, poi_id, region_id, spawn_room, spawn_pos)`. The picker is fired by the `interact` action (E key) when the ship is in range of a landable POI.",
...
# Whether a pack MUST ship a JSON file for this screen. Defaults to true so
# legacy screens stay required. Optional screens (e.g. `toast`) only render
# their authored template when present; absence is fine and the runtime
# falls back to a hardcoded default.
static func screen_is_required(screen_id: String) -> bool:
	return bool(SCREEN_DEFS.get(screen_id, {}).get("required", true))
