...
static func _stock_toast_screen() -> Dictionary:
    var root := _base_screen("toast", 240, 40)
    root["properties"]["padding"] = 6
    var msg := _label("toast_message", 8, 8, 224, 24, "", "body", "toast.message")
    msg["properties"]["wrap"] = true
    msg["properties"]["alignment"] = "left"
    root["children"] = [msg]
    return root
