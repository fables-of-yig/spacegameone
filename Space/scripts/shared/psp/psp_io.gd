...
static func save_as_new_preset(pack_id: String, preset_id: String, preset_name: String, preset_description: String, frames_data: Dictionary, poses_data: Dictionary) -> bool:
    var trimmed_id: String = preset_id.strip_edges()
    if trimmed_id.is_empty():
        push_error("PspIO: save_as_new_preset requires a non-empty preset id")
        return false
    if trimmed_id != _sanitized_preset_id(trimmed_id):
        push_error("PspIO: preset id '%s' must use lowercase letters, digits, and underscores only" % preset_id)
        return false
    var existing: Dictionary = load_preset(pack_id, trimmed_id)
    if not existing.is_empty():
        push_error("PspIO: preset id '%s' already exists; use OVERWRITE PRESET instead" % trimmed_id)
        return false

    var preset_dir: String = shipped_sprites_dir(pack_id) + PRESET_ROOT_DIR + "/" + trimmed_id + "/"
    _ensure_dir(preset_dir)
    ...
    var trimmed_name: String = preset_name.strip_edges()
    if trimmed_name.is_empty():
        trimmed_name = trimmed_id
    var manifest: Dictionary = {
        "id": trimmed_id,
        "name": trimmed_name,
        "description": preset_description.strip_edges(),
        "frames": "player_frames.json",
        "poses": "player_poses.json",
    }
    if not _write_json(preset_dir + "player_frames.json", out_frames):
        return false
    if not _write_json(preset_dir + "player_poses.json", out_poses):
        return false
    return _write_json(preset_dir + "preset.json", manifest)


static func _sanitized_preset_id(raw: String) -> String:
    var out: String = ""
...
static func _starter_frame_entries() -> Array:
    # Empty by design: leave every animation unfilled so authors fill them
    # in (or rely on the *_right/*_left mirror partner). Previously we
    # seeded every pose with `{sheet:"base", index:0}` which surfaced the
    # autogenerated robot starter frame in any animation the user hadn't
    # filled in yet.
    return [
...
static func _starter_pose_entries() -> Dictionary:
    ...
    poses["213"] = _starter_pose("parry_right", 1, 30, 16, 0, 24, [4, 4, 4, 4], -1, 1)
    poses["214"] = _starter_pose("parry_left", -1, 30, 16, 0, 24, [4, 4, 4, 4], -1, 2)
...
static func _create_starter_sheet_image() -> Image:
    ...
    for i in range(DEFAULT_SHEET_COLS * DEFAULT_SHEET_COLS):
        ...
        var cell_y := (i / DEFAULT_SHEET_COLS) * DEFAULT_FRAME_H
